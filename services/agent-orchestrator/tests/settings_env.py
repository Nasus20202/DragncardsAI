...
environment. A developer who narrows ``ENABLED_PROVIDER_IDS`` to the providers
they hold keys for, or who exports the rest of this service's configuration (for
example by sourcing ``.env``, or via ``uv run --env-file`` as ``scripts/test.sh``
does for integration runs), would otherwise change what the tests observe. Tests
