...
import pytest
from dragncards_common.schema_migrations import (
    _load_versioned_sql,
    _migration_applied_at,
    _split_statements,
    discover_migrations,
)
...
def test_migration_applied_at_depends_on_dialect():
    sqlite_value = _migration_applied_at("sqlite")
    postgres_value = _migration_applied_at("postgresql")
...
def test_load_versioned_sql_prefers_dialect_and_falls_back(tmp_path: Path):
    ...
    assert _load_versioned_sql(sql_dir, "0001_initial", "sqlite") == "sqlite version"
    assert _load_versioned_sql(sql_dir, "0002_extra", "postgresql") == "generic version"


def test_discover_migrations_reads_versions_from_sql_dir(tmp_path: Path):
    sql_dir = tmp_path / "sql"
    sql_dir.mkdir()
    ...
    assert discover_migrations(sql_dir) == ("0001_initial", "0002_extra")
