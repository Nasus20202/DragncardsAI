...
class StrictRequest(BaseModel):
    ...
    with the field silently discarded. DRA-60 is that failure in production: a
    client sending ``{"selection": {"round": [1]}}`` to an eval-service
    predating this base — ``round`` where the model declares ``rounds`` — had
    the unknown key dropped, so the server's own error said only "selection
    must specify at least one of: seqs, rounds, seq_range, whole_game" and
    never named the typo.
    ...
    a client newer than an eval-service that already carries this base; it can do
