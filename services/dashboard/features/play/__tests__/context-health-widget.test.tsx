...
vi.mock("@heroui/react", () => ({
  Button: ({ children, isDisabled, onPress }: { children: React.ReactNode; isDisabled?: boolean; onPress?: () => void }) => (
    <button disabled={isDisabled} onClick={onPress}>{children}</button>
  ),
  ProgressBar: ({ value, color, "aria-label": ariaLabel }: { value: number; color: string; "aria-label": string }) => (
    <div role="progressbar" aria-label={ariaLabel} data-value={value} data-color={color} />
  ),
...
describe("ContextHealthWidget", () => {
  it("renders null when contextMetadata is null", () => {
    const { container } = render(
      <ContextHealthWidget contextMetadata={null} isBusy={false} onCompact={vi.fn()} />
    );
  ...
  it("shows progress bar with correct percentage (below 70%)", () => {
    render(
      <ContextHealthWidget contextMetadata={baseMetadata} isBusy={false} onCompact={vi.fn()} />
    );
  ...
  it("colors progress bar amber between 70% and 85%", () => {
    const metadata: ContextMetadata = { ...baseMetadata, usage_ratio: 0.75, tokens_used: 96000 };
    render(
      <ContextHealthWidget contextMetadata={metadata} isBusy={false} onCompact={vi.fn()} />
    );
    const bar = screen.getByRole("progressbar");
  ...
  it("colors progress bar red above 85%", () => {
    const metadata: ContextMetadata = { ...baseMetadata, usage_ratio: 0.9, tokens_used: 115200 };
    render(
      <ContextHealthWidget contextMetadata={metadata} isBusy={false} onCompact={vi.fn()} />
    );
    const bar = screen.getByRole("progressbar");
  ...
  it("shows Memory off state and no progress bar when multi_turn_memory is false", () => {
    const metadata: ContextMetadata = { ...baseMetadata, multi_turn_memory: false };
    render(
      <ContextHealthWidget contextMetadata={metadata} isBusy={false} onCompact={vi.fn()} />
    );
  ...
  it("disables Compact button while busy", () => {
    render(
      <ContextHealthWidget contextMetadata={baseMetadata} isBusy={true} onCompact={vi.fn()} />
    );
  ...
  it("calls onCompact when Compact button is clicked", () => {
    ...
    render(
      <ContextHealthWidget contextMetadata={baseMetadata} isBusy={false} onCompact={onCompact} />
    );
