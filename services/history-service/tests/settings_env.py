...
environment. A developer who exports this service's configuration -- for example
by sourcing ``.env``, or via ``uv run --env-file`` as ``scripts/test.sh`` does for
integration runs -- would otherwise change what the tests observe. Tests
