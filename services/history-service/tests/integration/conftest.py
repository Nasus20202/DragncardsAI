...
POSTGRES_URL = os.environ.get(
    ...
    "postgresql+asyncpg://postgres:postgres@localhost:5442/history_service",
)
VALKEY_URL = os.environ.get("VALKEY_URL", "redis://localhost:6381/0")
...
async def _valkey_available() -> bool:
    conn = RespConnection.from_url(VALKEY_URL)
    try:
        return (await conn.execute("PING")) in ("PONG", "pong")
    except Exception:
        return False
...
async def postgres_repository():
    ...
    database_name = f"history_service_test_{uuid4().hex}"
...
async def valkey_connection():
    if not await _valkey_available():
        pytest.skip("Valkey not reachable for integration tests")
    conn = RespConnection.from_url(VALKEY_URL)
    yield conn
    await conn.aclose()
