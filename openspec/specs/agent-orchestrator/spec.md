...
Before building the messages list for a new job, when `multi_turn_memory` is enabled, the system SHALL estimate the replay context size by reconstructing the message history (using the same tiktoken estimation used by the context metadata endpoint) and computing the ratio against the model's context window. If the ratio exceeds `CONTEXT_COMPACTION_THRESHOLD`, the system SHALL compact automatically before proceeding.

The window used as the denominator SHALL be the provider-reported context length for the session's model, falling back to the configured `CONTEXT_WINDOW_SIZE` only when the provider reports none. A configured value SHALL NOT override what the provider reports, because one deployment serves models whose real windows differ by more than an order of magnitude.
...
Threshold is configured via `CONTEXT_COMPACTION_THRESHOLD` env var (float, default `0.8`). The context-window fallback is `CONTEXT_WINDOW_SIZE` (int, default `128000`).
