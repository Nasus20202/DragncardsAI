...
`GameStateSnapshot`, dual-used as the request body of `PUT /games/{session_id}/snapshot` and as the response of the matching export endpoint, SHALL inherit the strict base as well: the export route constructs the snapshot with exactly the model's three declared fields, so the response side tolerates the rule, and the import request side requires it (the structural walk reaches it, so leaving it on `BaseModel` would fail the structural test).
...
- **THEN** the Game Service SHALL respond `422` and the response SHALL name `dest_stack_indexx` as the unknown field
...
- **THEN** the Game Service SHALL respond `422` and the response SHALL name that key
...
- **WHEN** a client sends a request body that contains an open-mapping field the Game Service declares with an arbitrary contents shape
- **THEN** the Game Service SHALL accept the request and pass that field's contents through untouched
