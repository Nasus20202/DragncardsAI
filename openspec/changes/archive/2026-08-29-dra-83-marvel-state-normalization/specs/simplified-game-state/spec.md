...
The neutral `villainHitPoints` field SHALL represent the total hit points of the active
villain stage across platforms. For marvel-lcg, when the active villain's `info.health`
represents current remaining health, the normalizer SHALL reconstruct the stage total as
`info.health + damage` (where damage is read from `c_damage`, `k_damage`, or `damage`), while
exposing the damage counter as `tokens.damage` on the card in `sharedVillain`, so that
consumers derive remaining health as `villainHitPoints - sharedVillain[0].tokens.damage`.
