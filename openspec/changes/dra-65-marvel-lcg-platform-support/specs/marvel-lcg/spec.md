# marvel-lcg Integration Contract
...
marvel-lcg SHALL create a game entirely over HTTP through `GET /new?data=<NewGameDescriptor>`, where
the descriptor is a JSON object with the fields `campaign_json`, `encounter_set_names`, `hero_json`,
`seed`, `timeout`, `challenges`, `rules` and `campaign_log`. It SHALL answer a successful creation
`200` with `{"result": "New game created"}`, an invalid descriptor `400` with an `error` field, and a
conflicting campaign progression `409`.

`campaign_json` and `hero_json` SHALL carry the **stringified JSON content** of the scenario and deck
documents, not their file paths. `hero_json` SHALL be a list holding one such string per hero. Our
client SHALL therefore discover the documents first — `GET /list_scenarios` and
`GET /list_starter_deck` return path listings, and `GET /get_scenario_json?<path>` and
`GET /get_hero_json?<path>` return the document text for a listed path — and SHALL embed the returned
text as the descriptor's field value. Passing a path where content is required SHALL be treated as a
client defect, not as an alternative form.

Our client SHALL set up games only from the scenarios and decks that ship in the vendored engine, and
SHALL NOT use the engine's MarvelCDB import paths.

#### Scenario: A solo game is created from fetched document content
- **WHEN** our client creates a solo game
- **THEN** it SHALL first fetch the scenario document via `GET /get_scenario_json?<path>` and the hero deck document via `GET /get_hero_json?<path>`
- **AND** it SHALL send `GET /new` with `campaign_json` set to the scenario document's text and `hero_json` set to a list containing the deck document's text
...
- **WHEN** a descriptor is constructed with `campaign_json` set to a file path rather than the document's text
- **THEN** our client SHALL fail that construction before sending it
- **AND** SHALL NOT rely on the engine to interpret the path

#### Scenario: An invalid descriptor is reported by status
- **WHEN** our client sends `GET /new` with a descriptor the engine rejects
- **THEN** the engine SHALL answer `400` with an `error` field, or `409` when a campaign progression conflicts
- **AND** our client SHALL surface that status as a table-creation failure rather than retrying blindly

#### Scenario: Only vendored decks and scenarios are used
- **WHEN** our client lists the scenarios and decks available for a game
- **THEN** the listings SHALL come from `GET /list_scenarios` and `GET /list_starter_deck` on the vendored engine
- **AND** our client SHALL NOT call any MarvelCDB import or sync path
