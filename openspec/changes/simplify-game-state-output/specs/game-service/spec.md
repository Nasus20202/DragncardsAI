...
The Game Service SHALL provide endpoints and MCP tools to query the current game state for a given session, including an optional simplified format for Marvel Champions sessions.
...
- **WHEN** a client sends `GET /games/{id}/state?format=simplified` for a Marvel Champions session
...
- **WHEN** a client requests simplified state
