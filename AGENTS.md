...
- Commits must follow the semantic commit convention (e.g. `feat:`, `fix:`, `refactor:`, `chore:`, `test:`, `docs:`, `style:`, `ci:`, or scoped variants like `feat(orchestrator):`), use a single-line message, and must NOT include any AI attribution (no `Co-Authored-By` trailer naming an assistant).
...
- **An issue is closed only once the OpenSpec change carrying its requested changes has been archived.** Ticked tasks are an agent's own account of the work; archiving is what writes the change's spec deltas into `openspec/specs/`, and until that happens the durable record still says the behaviour is merely proposed. Closing the issue first publishes a contradiction — the tracker says shipped, the specs say not yet — and the tracker is what the owner reads. This is not hypothetical: DRA-19 was set to `Done` while its change was deliberately left active with three sections unbuilt, and the owner reopened it with *"move back to in progress, not finished."* So: archive the change first, confirm it actually landed by listing `openspec/changes/archive/` rather than trusting a command's exit code, and only then move the issue to `Done`. If the archive cannot complete — a placeholder still needs real prose, tasks remain unchecked, or the owner has not agreed the feature is finished — leave the issue open and report why rather than closing it anyway. Where one change carries several issues, none of them close until that change is archived.
...
- Squash commit subjects follow the semantic commit convention and end with ` (DRA-<n>)` (e.g. `feat(orchestrator): enforce game-service session binding (DRA-79)`) — still a single line, still no AI attribution.
